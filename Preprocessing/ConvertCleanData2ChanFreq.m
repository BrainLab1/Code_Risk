...
% 24.09.2017 modified by Saeed
% Initiated from 'powspctrm_clean_data.m' and 'fieldtrip_freqanalysis_session.m' by Amin Nejatbakhsh
...
for i = 1: length(file_names) % for each session
    try
        ...
        for ch = 1: length(data.label) % for each channel
            ...
            LFP_outliers = cellfun(@(x,y) unique([x';y]), {event.bad_lfp_channels}, {event.bad_amplitude_channels},'UniformOutput',0)';
