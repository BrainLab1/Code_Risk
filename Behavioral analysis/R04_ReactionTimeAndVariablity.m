...
for ses = 1:numel(allFiles) % for each session
    ...
    for gr = 1:length(output)
        ...
        for tr = 1:length(output(gr).SuccessTrlIdx) % for each success trial of this group
            ...
            % read out the transformed diode-reaction time for this successful trial of this group
            tempDRT = [tempDRT; eventTable.z_log_RT(trIdx)];
            clear trIdx
        end
        output(gr).TrnsfrmDRT   = tempDRT; 
        output(gr).TrnsfrmDRTvar = var(tempDRT);
        clear tempDRT tr 
    end
    clear gr 
