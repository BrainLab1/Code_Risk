% last update: 10.03.2018: new condition was added for 'expected_reward'
...
switch grType
    ...
    case 'expected_reward'
        ...
        output = [];
        for i=1:length(possibleVals)
            output = [output; struct('TrialIdx', find(eventTable.expected_reward, possibleVals(i)), ...
                                     'Value', possibleVals(i), ...
   ...
   case '18 task conditions' % grop trial by : ExpectedRewardValue & RewardVariance & cue-target Congruency 
    ...
    case 'CuePos & CurrEV & CurrVar' 
        % make a matrix with all posiible combinations of these variables
        allConditions = combvec([-1 1],[3 6 9],[0 1 4])';
        output = [];
        eventTable = struct2table(event);
        for gr = 1:size(allConditions,1)
            trlIdx = find((eventTable.cue_pos==allConditions(gr,1)) .* (eventTable.expected_reward==allConditions(gr,2)) .* (eventTable.RewardVariance==allConditions(gr,3)));
            output = [output; struct('TrialIdx', trlIdx, ...
                                     'Value', allConditions(gr,:), ...
                                     'GroupingType', grType)];
            clear trlIdx                                 
    ...
    case 'CuePos & TargetPos & PreGood & CurrVar (uncertainty)' 
        % make a matrix with all possible combinations of these variables
        allConditions = combvec([-1 1],[-1 1], [0,1] ,[0 1])'; % we assign variance 0 to R50* and variance 1 to C50* trial types 
        output = [];
        eventTable = struct2table(event);
        rewardVariance = cellfun(@(x) (strcmp(x(1),'R')*0)+(strcmp(x(1),'C')*1), eventTable.type);
        for gr = 1:size(allConditions,1)
            trlIdx = find((eventTable.cue_pos==allConditions(gr,1)) .* (eventTable.target_pos==allConditions(gr,2)) .* ...
                          (eventTable.pre_good==allConditions(gr,3)) .* (rewardVariance==allConditions(gr,4)));
            output = [output; struct('TrialIdx', trlIdx, ...
                                     'Value', allConditions(gr,:), ...
                                     'GroupingType', grType)];
            clear trlIdx                                 
    ...
    case 'CuePos & TargetPos & CurrEV & CurrVar & PreGood (uncertainty all conditions)' 
        % make a matrix with all possible combinations of these variables
        allConditions = combvec([-1 1],[-1 1], [0 0.25 0.5 0.75 1], [0 0.1875 0.25], [0 1])'; % variances were measured based on min_reward_value=0 and max_reward_value=1; 
        output = [];
        eventTable = struct2table(event);
        rewardVariance = nan(size(eventTable,1), 1);
        rewardVariance = cellfun( @(x) ((strcmp(x(1),'R') || strcmp(x(1:end-1),'C100') || strcmp(x(1:2),'C0')) * 0) + ...
                                        (strcmp(x(1:3),'C50')*0.25) + ((strcmp(x(1:3),'C25') || strcmp(x(1:3),'C75'))*0.1875), ...
                                         eventTable.type );
        for gr = 1:size(allConditions,1)
            trlIdx = find((eventTable.cue_pos==allConditions(gr,1)) .* (eventTable.target_pos==allConditions(gr,2)) .* ...
                          (eventTable.expected_reward==allConditions(gr,3)) .* (rewardVariance==allConditions(gr,4)) .* (eventTable.pre_good==allConditions(gr,5)));
