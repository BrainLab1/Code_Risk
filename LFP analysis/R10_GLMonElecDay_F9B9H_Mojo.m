...
num_ch = 96;
minElecRepPerCnd = 10;
timeOfInt = [{[0.2 0.4]}];  % time in sec. based on Cue alignment
freqOfInt = [{[12 20]}]; % frequency in hertz
...
%     freq_data_dir = 'H:\Risk Task\MacDuff\'; %Bahareh PC
    ...
    save_result_dir = 'X:\Report 10\GLM on elec-days_Risk_F9B9H\MacDuff\';
...
%     freq_data_dir = 'F:\Electrode-day power spectrum\Risk Task\Mojo\';% Saeed PC
    save_result_dir = 'X:\Report 10\GLM on elec-days_Risk_F9B9H\Mojo\';
end
...
for ses = 1:length(monkeyCleanData) 
    ...
    trlIdx2Conditions = find(cellfun(@(x) strcmp(x,'B9H')+strcmp(x,'F9'),{event.type}'));
...
% % % %             allFreqTimIdx = combvec(fIdx',tIdx')';
        ...
        % keep only conditions "F9" and "B9H" from the event structure
        event = event(trlIdx2Conditions);
        % keep only conditions "F9" and "B9H" from the power spectrum
...
for rw = 1:numRows
    for cl = 1:numClmn
        ...
        plot(mean(allSampleEV)*[1 1], [0 .2], 'r--')
        text(mean(allSampleEV), 0.1,  ['\color[rgb]{'  num2str([1 0 0]) '} mean=' num2str(mean(allSampleEV))], 'Rotation', 90)
        plot(mean(allSampleVar)*[1 1], [0 -.2], 'r--')
        text(mean(allSampleVar), -0.1,  ['\color[rgb]{'  num2str([1 0 0]) '} mean=' num2str(mean(allSampleVar))], 'Rotation', 90)
        
        txt = [];
        txt = [txt; {['\color[rgb]{'  num2str([0 0 1]) '} expected value']} ];
        txt = [txt; {['\color[rgb]{' num2str([0 0 1]) '} significants ' num2str(sum(signiBinCountsEv)) '/' num2str(sum(binCountsEV))]} ];
        [~,p_EV] = ttest(allSampleEV);
        txt = [txt; {['\color[rgb]{' num2str([0.3 0.3 0.3]) '} ttest p value ' num2str(p_EV)]} ];
        text(0.2, 0.1, txt) 
        
        txt = [];
        txt = [txt; {['\color[rgb]{'  num2str([1 0 1]) '} variance']} ];
        txt = [txt; {['\color[rgb]{' num2str([1 0 1]) '} significants ' num2str(sum(signiBinCountsVar)) '/' num2str(sum(binCountsVar))]} ];
        [~,p_Var] = ttest(allSampleVar);
        txt = [txt; {['\color[rgb]{' num2str([0.3 0.3 0.3]) '} ttest p value ' num2str(p_Var)]} ];
        text(0.2, -0.1, txt) 
