...
num_ch = 96;
minElecRepPerCnd = 10;
timeOfInt = [{[0.25 0.4]}];  % time in sec. based on Cue alignment
freqOfInt = [{[15 25]}]; % frequency in hertz
...
%     freq_data_dir = 'H:\Risk Task\MacDuff\'; %Bahareh PC
    ...
    save_result_dir = 'D:\Shared\Report 10\GLM on elec-days_Risk_F9B9H\MacDuff\';
...
%     freq_data_dir = 'H:\Risk Task\Mojo\';% Bahareh PC
    ...
    save_result_dir = 'D:\Shared\Report 10\GLM on elec-days_Risk_F9B9H\Mojo\';
end
...
% % % %             allFreqTimIdx = combvec(fIdx',tIdx')';
        ...
        while gr<length(validGrpForThisCh) % for all groups in which this electrode was valid for analysis (i.e., 1,...,9)
            ...
            thisChGLMpredictor{gr,ch} = repmat(output(grpIdx).ZValue(2),  length(thisChValGrpTrIdx) , 1);
...
for rw = 1:numRows
    for cl = 1:numClmn
        ...
        plot(mean(allSampleEV)*[1 1], [0 .04], 'r--')
        text(mean(allSampleEV), 0.01,  ['\color[rgb]{'  num2str([1 0 0]) '} mean=' num2str(mean(allSampleEV))], 'Rotation', 90)
        ...
        set(gca, 'YLim', [0 0.04], 'YTick', [-0.2 -0.1 0 0.1 0.2], 'YTickLabel', {'20', '10', '0', '10', '20'}, 'XTick', [-0.4 :0.2: 0.4], 'XTickLabel', {'-0.4', '-0.2', '0', '0.2', '0.4'})
        plot([0 0], [-.2 .2], 'k')
        plot([-0.5 0.5], [0 0], 'k')
