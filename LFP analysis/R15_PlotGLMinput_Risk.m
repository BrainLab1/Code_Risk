...
% 30.04.2018: This is the same scripts as 'R10_PlotGLMinput_Risk'. We only chaged the electrode mapping
...
ROI.freq  = [{[10 20]} , {[ 4   10]} , {[0  20]}];
ROI.time  = [{[0 0.4]} , {[0.4 0.8]} , {[0 0.4]}];
...
switch Monkey 
    case 'MacDuff'
        ...
        load_dir = 'X:\Report 15\GLM analysis\MacDuff\GLM input data\';
        save_dir = 'X:\Report 15\GLM analysis\MacDuff\GLM input data to plot\';
    case 'Mojo'
        bhv_data_dir  = 'Z:\data\Risk\Behavior\';
        freq_data_dir = 'H:\Risk Task\Mojo\';
        intermediate_save_folder = 'D:\quick hack_Mojo\';
        load_dir = 'X:\Report 15\GLM analysis\Mojo\GLM input data\';
        save_dir = 'X:\Report 15\GLM analysis\Mojo\GLM input data to plot\';
end
...
while roi < length(ROI.event) % for each frequency
    ...
    for vr=1:length(uniqueVar)
        ...
        eval(['txt_2D_EV = [txt_2D_EV; {[''\color[rgb]'' colorsString{vr} '' Var= '' num2str(uniqueVar(vr).^2)]}];'])
    end
    clear vr
    set(gca, 'XLim', [0 4],  'XTick', [1 2 3], 'XTickLabel', [{'3'},{'6'},{'9'}])
    text(1, mean(ylim), txt_2D_EV)

    % plot 2D_Power vs Var
    uniqueEV = unique(xData)';
    figure(fg3)
    subplot(numRows, numCulomns, roi), hold on, box on
    title([ {['ROI: [' num2str(ROI.freq{roi}(1)) ' ' num2str(ROI.freq{roi}(2)) '] Hz']},{['   ' ROI.event{roi} ' [' num2str(ROI.time{roi}(1)) ' ' num2str(ROI.time{roi}(2)) '] sec']} ])
    xlabel(['Variance'])
    ...
    for ev=1:length(uniqueEV)
        errorbar(yData(ev,:), dataChart{roi}(ev,:), errorChart{roi}(ev,:), 'Color', colorsNumeric{ev}, 'LineWidth', 1.5)  % each colunm of xData correponds to one variance level!
        eval(['txt_2D_Var = [txt_2D_Var; {[''\color[rgb]'' colorsString{ev} '' EV= '' num2str(3*uniqueEV(ev))]}];'])
    end
    clear ev
    set(gca, 'XLim', [-1 3],  'XTick', [0 1 2], 'XTickLabel', [{'0'},{'1'},{'4'}])
    text(1, mean(ylim), txt_2D_Var)
