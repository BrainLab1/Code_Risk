...
%         freq_data_dir = 'H:\MacDuff\';
        ...
        intermediate_save_folder = 'D:\Shared\quick hack_MacDuff\';
...
%         freq_data_dir = 'F:\Electrode-day power spectrum\Risk Task\Mojo\';
        intermediate_save_folder = 'D:\Shared\quick hack_Mojo\';        
        save_dir = 'X:\Report 10\GLMon2ConditionRisk\Mojo\F9_B9H\';
        load_dir = '';
end
...
% 8 is numOfGroups for groupingCriteria  = 'CuePos & TargetPos & CurrEV & CurrVar' for F9 and B9H conditions;
numGroups = 8;
allSesActivityArr1 = cell(numGroups,1);
allSesActivityArr2 = cell(numGroups,1);
...
for ses = 1:length(all_bhv_folder_names)
    tempActivity = cell(numGroups,1);  % 8 is numOfGroups for groupingCriteria  = 'CuePos & TargetPos & CurrEV & CurrVar';
     ...
     if strcmp(selectedArray, Array_pos{arrayIdx})   
        ...
        switch groupingCriteria
            ...
            otherwise
                ...
                % keep only conditions "F9" and "B9H" from the normalized power spectrum
...
for gr = 1:length(output)
    ...
    % since we only kept F9 and B9H conditions, remove the expected value as an independent parameter
...
saveas(fg1, [save_dir Monkey '_GLM_F9-B9H_' selectedArray '_' date], 'fig')
saveas(fg1, [save_dir Monkey '_GLM_F9-B9H_' selectedArray '_' date], 'bmp')
close(fg1)
