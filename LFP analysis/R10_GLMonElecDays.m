...
num_ch = 96;
minElecRepPerCnd = 10;
timeOfInt = [{[0.1 0.3]}, {[0.25 0.4]} , {[0.4 0.6]}];  % time in sec. based on Cue alignment
freqOfInt = [{[4 8]},    {[15 25]} , {[4 8]}]; % frequency in hertz
...
%     freq_data_dir = 'F:\Electrode-day power spectrum\MacDuff\';% Saeed PC
    save_result_dir = 'X:\Report 9\GLM on elec-days\MacDuff\';
...
%     freq_data_dir = 'F:\Electrode-day power spectrum\Mojo\';% Saeed PC
    save_result_dir = 'X:\Report 9\GLM on elec-days\Mojo\';
end
...
for rw = 1:numRows
    for cl = 1:numClmn
        ...
        text(0.05, 0.07, ['\color[rgb]{' num2str([0.3 0.3 0.3]) '} ttest p value ' num2str(p_EV)])
        ...
        text(0.05, -0.17, ['\color[rgb]{' num2str([0.3 0.3 0.3]) '} ttest p value ' num2str(p_Var)])
