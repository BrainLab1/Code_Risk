...
for ses = 1:length(all_bhv_folder_names) 
    ...
    dummy = squeeze(mean(reshape(cell2mat(elec_day_array1), numGroups*length(freq.freq), length(freq.time), []), 3));
    colorScaleElecAve1 = [min(dummy(:)) max(dummy(:))];
    clear dummy 
    dummy = squeeze(mean(reshape(cell2mat(elec_day_array2), numGroups*length(freq.freq), length(freq.time), []), 3));
    ...
    for gr = 1:numGroups
        ...
        Elec_mean_array1{gr,ses} = squeeze(mean(reshape(cell2mat(elec_day_array1(gr,:)), numel(freq.freq), length(freq.time), []), 3));% mean across electrodes
        ...
        % ------ plot Array 2 -------
        Elec_mean_array2{gr,ses} = squeeze(mean(reshape(cell2mat(elec_day_array2(gr,:)),numel(freq.freq),length(freq.time),[]),3));% mean across electrodes
    ...
    %% Add the mean power from this session to the sum of the power spectrums from all sessions, separately for the two arrays and electrodes
    Ses_sum1 = Ses_sum1 + cell2mat(elec_day_array1);
    Ses_sum2 = Ses_sum2 + cell2mat(elec_day_array2);
...
for gr = 1:numGroups
    ...
    % extract current group from the sum
    Ses_avg_gr_array1 = Ses_sum1((gr-1)*length(freq.freq)+1 : gr*length(freq.freq), :)/ses_num;
    Ses_avg_gr_array2 = Ses_sum2((gr-1)*length(freq.freq)+1 : gr*length(freq.freq), :)/ses_num;
    
    for ch = 1:48
        % extract channel index 'ch' on the array 
        Ses_avg_elec_array1 = Ses_avg_gr_array1(:,(ch-1)*length(freq.time)+1 : ch*length(freq.time));
        Ses_avg_elec_array2 = Ses_avg_gr_array2(:,(ch-1)*length(freq.time)+1 : ch*length(freq.time));
...
for gr = 1:numGroups
    ...
    sesAveArray2 = squeeze( mean(reshape(elecAveArray2, size(elecAveArray2,1), length(freq.time), []), 3) );
