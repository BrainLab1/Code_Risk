...
% 16.05.2018 previously the interaction term was not included in the GLM analysis! this is the same GLM script as before with ne electrode map and interaction!
...
%         intermediate_save_folder = 'D:\quick hack\';
        save_dir = 'D:\Shared\Report 17\GLM analysis\MacDuff\LFP GLM input data\';
    case 'Mojo'
        bhv_data_dir  = 'Z:\data\Risk\Behavior\';
...
paraNames = {'CuePos', 'TargetPosition', 'CurrEV', 'CurrVar', 'IntEV*Var','Noise'};
fg1= figure('Name',Array_pos{1}); fg2= figure('Name',Array_pos{2});
colorRange = [-0.12, 0.08];

% change b and stats into proper format 
c = mat2cell( cell2mat( cellfun(@(x) cell2mat(x), b, 'UniformOutput', 0) ), repmat(length(paraNames)+1, numFreq,1), ones(1,numTime) );

for para = 2:length(paraNames)+1 % for each parameter except for the constant value!
    
    temp = cellfun(@(x) x(para), c, 'UniformOutput', 0);
