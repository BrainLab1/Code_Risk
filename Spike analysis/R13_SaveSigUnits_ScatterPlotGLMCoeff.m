...
while ses <= length(sesListSpkCnt)
    ses
    % get the list of active units in this session on array 1
    thisSesActiveUnits = ActiveSesUnitsArr1( find( strcmp(ActiveSesUnitsArr1(:,1),sesListSpkCnt{ses}) ) , 2);
    
    % load this sesion grouped spike count file; this will load spkDataArr1 and spkDataArr2
    ...
    for un=1:length(thisSesActiveUnits)
        if ~isempty (spkDataArr1.(thisSesActiveUnits{un,1}{1,1}))
            allUnitIDsArray1 = [allUnitIDsArray1; [{sesListSpkCnt{ses}},{thisSesActiveUnits{un,1}{1,1}}]];
            popUnitIdxArr1 = popUnitIdxArr1 + 1;
            sz = cellfun(@(x) length(x), {spkDataArr1.(thisSesActiveUnits{un,1}{1,1}).SpikeCount}');
            thisUnitParaVal = cellfun( @(x,y) repmat(x, y, 1), {spkDataArr1.(thisSesActiveUnits{un,1}{1,1}).Value}', mat2cell(sz, ones(length(sz),1) ,1) , 'UniformOutput' , 0 ); 
            thisUnitParaVal = cell2mat(thisUnitParaVal);
            clear sz
            
            % collect firing rate of the neuron, trial-by-trial
            thisUnitSpkCount = cell2mat({spkDataArr1.(thisSesActiveUnits{un,1}{1,1}).SpikeCount}'); % each row will be the spike count for one unit
    ...
    clear thisSesActiveUnits un

    % collect spike counts from all the units on array 2
    % get the list of active units in this session on array 2
    thisSesActiveUnits = ActiveSesUnitsArr2( find( strcmp(ActiveSesUnitsArr2(:,1),sesListSpkCnt{ses}) ) , 2);
    for un=1:length(thisSesActiveUnits)
        if ~isempty (spkDataArr2.(thisSesActiveUnits{un,1}{1,1}))
            allUnitIDsArray2 = [allUnitIDsArray2; [{sesListSpkCnt{ses}},{thisSesActiveUnits{un,1}{1,1}}]];
            popUnitIdxArr2 = popUnitIdxArr2 + 1;
            sz = cellfun(@(x) length(x), {spkDataArr2.(thisSesActiveUnits{un,1}{1,1}).SpikeCount}');
            thisUnitParaVal = cellfun( @(x,y) repmat(x, y, 1), {spkDataArr2.(thisSesActiveUnits{un,1}{1,1}).Value}', mat2cell(sz, ones(length(sz),1) ,1) , 'UniformOutput' , 0 ); 
            thisUnitParaVal = cell2mat(thisUnitParaVal);
            clear sz
            
            % collect firing rate of the neuron, trial-by-trial
            thisUnitSpkCount = cell2mat({spkDataArr2.(thisSesActiveUnits{un,1}{1,1}).SpikeCount}'); % each row will be the spike count for one unit
            % zscore firing rates of this neuron, accross trials, take care of the NaN values
            aa = zscore(thisUnitSpkCount(~isnan(thisUnitSpkCount)));
            thisUnitSpkCount(~isnan(thisUnitSpkCount)) = aa;
            clear aa
...
% % % %             allUnitIDsArray1 = [allUnitIDsArray1; [{sesListSpkCnt(ses).name},{unNames{un}}]];
% % % %             popUnitIdxArr1 = popUnitIdxArr1 + 1;
% % % %             sz = cellfun(@(x) length(x), {spkDataArr1.(unNames{un}).SpikeCount}');
% % % %             thisUnitParaVal = cellfun( @(x,y) repmat(x, y, 1), {spkDataArr1.(unNames{un}).Value}', mat2cell(sz, ones(length(sz),1) ,1) , 'UniformOutput' , 0 ); 
% % % %             thisUnitParaVal = cell2mat(thisUnitParaVal);
% % % %             clear sz
% % % %             
% % % %             % collect firing rate of the neuron, trial-by-trial
% % % %             thisUnitSpkCount = cell2mat({spkDataArr1.(unNames{un}).SpikeCount}'); % each row will be the spike count for one unit
% % % %             % zscore firing rates of this neuron, accross trials, take care of the NaN values
% % % %             aa = zscore(thisUnitSpkCount(~isnan(thisUnitSpkCount)));
% % % %             thisUnitSpkCount(~isnan(thisUnitSpkCount)) = aa;
% % % %             clear aa
...
% % % %             allUnitTrlsSpkCountArr1 = [allUnitTrlsSpkCountArr1; thisUnitSpkCount];
% % % %             allUnitTrlsParaValArr1  = [allUnitTrlsParaValArr1;  thisUnitParaVal ];
% % % %             allUnitTrlsUnitIdxArr1  = [allUnitTrlsUnitIdxArr1;  repmat([popUnitIdxArr1 ses], size(thisUnitParaVal,1),1)];
% % % %             
% % % %             clear thisUnitParaVal thisUnitSpkCount
% % % %         end
% % % %     end
...
% % % %             allUnitIDsArray2 = [allUnitIDsArray2; [{sesListSpkCnt(ses).name},{unNames{un}}]];
% % % %             popUnitIdxArr2 = popUnitIdxArr2 + 1;
% % % %             sz = cellfun(@(x) length(x), {spkDataArr2.(unNames{un}).SpikeCount}');
% % % %             thisUnitParaVal = cellfun( @(x,y) repmat(x, y, 1), {spkDataArr2.(unNames{un}).Value}', mat2cell(sz, ones(length(sz),1) ,1) , 'UniformOutput' , 0 ); 
% % % %             thisUnitParaVal = cell2mat(thisUnitParaVal);
% % % %             clear sz
% % % %             
% % % %             % collect firing rate of the neuron, trial-by-trial
% % % %             thisUnitSpkCount = cell2mat({spkDataArr2.(unNames{un}).SpikeCount}'); % each row will be the spike count for one unit
% % % %             % zscore firing rates of this neuron, accross trials, take care of the NaN values
% % % %             aa = zscore(thisUnitSpkCount(~isnan(thisUnitSpkCount)));
% % % %             thisUnitSpkCount(~isnan(thisUnitSpkCount)) = aa;
% % % %             clear aa
...
% % % %             allUnitTrlsSpkCountArr2 = [allUnitTrlsSpkCountArr2; thisUnitSpkCount];
% % % %             allUnitTrlsParaValArr2  = [allUnitTrlsParaValArr2;  thisUnitParaVal ];
% % % %             allUnitTrlsUnitIdxArr2  = [allUnitTrlsUnitIdxArr2;  repmat([popUnitIdxArr2 ses], size(thisUnitParaVal,1),1)];
% % % %             
% % % %             clear thisUnitParaVal thisUnitSpkCount
% % % %         end
% % % %     end
...
save([SpkCountsPath '\Results\Active Units\' Monkey '_SigUnitIDs_dlPFC_' alignedEvent(2:end-1) ], ...
...
saveas(fg1, [SpkCountsPath  'Results\Active Units\' Monkey '_SignificantUnits_GLMcoeffScatter_dlPFC'], 'fig')
saveas(fg1, [SpkCountsPath  'Results\Active Units\' Monkey '_SignificantUnits_GLMcoeffScatter_dlPFC'], 'bmp')
close(fg1)
...
allSigUnit = unique([evSigUnitIdx; varSigUnitIdx; intSigUnitIdx; cueSigUnitIdx]);
% store significant unit IDs on the hard drive
save([SpkCountsPath '\Results\Active Units\' Monkey '_SigUnitIDs_7a_' alignedEvent(2:end-1) ], ...
...
saveas(fg2, [SpkCountsPath  'Results\Active Units\' Monkey '_SignificantUnits_GLMcoeffScatter_7a'], 'fig')
saveas(fg2, [SpkCountsPath  'Results\Active Units\' Monkey '_SignificantUnits_GLMcoeffScatter_7a'], 'bmp')
close(fg2)
clear evSigUnitIdx varSigUnitIdx intSigUnitIdx allSigUnit cueSigUnitIdx
