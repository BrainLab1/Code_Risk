...
addpath(genpath('D:\Dropbox\Shared_Jackie Project\temp code\dPCA_master_Kobak\matlab'))

Monkey = 'Mojo';
Area = '7a';
Intrvl = [0,4];
load([Monkey '_' Area '_[' num2str(Intrvl(1)) ',' num2str(Intrvl(2)) ']_allUnitsPSTH_Arr_Base'])

N   = size(allUnitsPSTH_Arr_Base{1},1);    % number of neurons
T   = size(allUnitsPSTH_Arr_Base{1},2);     % number of time points
EV  = 3;      % number of stimuli (number of expected conditions)
Var = 3;      % number of decisions (number of variance conditions)
E   = 1;     % maximal number of trial repetitions

firingRatesAverage = cell2mat(allUnitsPSTH_Arr_Base);

% % % % % % generating three components
% % % % % time = (1:T) / 10;
% % % % % component{1} = bsxfun(@times, ones(1,EV,Var,T,E), shiftdim(sin(time), -2)) * 20;
% % % % % component{2} = bsxfun(@times, ones(1,EV,Var,T,E), (1:EV)-ceil(EV/2)) .* bsxfun(@times, ones(1,EV,Var,T,E), shiftdim(time, -2));
% % % % % component{3} = bsxfun(@times, ones(1,EV,Var,T,E), shiftdim([-1 1],-1)) .* ...
% % % % %     bsxfun(@times, ones(1,EV,Var,T,E), shiftdim([time(1:end/2)*0 time(1:end/2)], -2)) * 6;
% % % % % 
...
% % % % % firingRates = reshape(firingRates, [N EV Var T E]);
% % % % % 
% % % % % % setting random number of repetitions for each neuron and condition
% % % % % trialNum = randi([5 E], [N EV Var]);
% % % % % for n = 1:N
% % % % %     for s = 1:EV
% % % % %         for d = 1:Var
...
decodingClasses = {[(1:EV)' (1:EV)'], repmat([1:2], [EV 1]), [], [(1:EV)' (EV+(1:EV))']};
